# THIS FILE IS SAFE TO EDIT. It will not be overwritten when rerunning go-raml.

import json as JSON
import os

import jsonschema
from flask import jsonify, request
from jsonschema import Draft4Validator

dir_path = os.path.dirname(os.path.realpath(__file__))
User_schema = JSON.load(open(dir_path + "/schema/User_schema.json"))
User_schema_resolver = jsonschema.RefResolver(
    "file://" + dir_path + "/schema/", User_schema
)
User_schema_validator = Draft4Validator(
    User_schema, resolver=User_schema_resolver
)


def drones_postHandler():

    inputs = request.get_json()

    try:
        User_schema_validator.validate(inputs)
    except jsonschema.ValidationError as e:
        return jsonify(errors="bad request body"), 400

    return jsonify()
