...
class deliveries_bydeliveryIdView(HTTPMethodView):
    async def get(self, request, deliveryId):
        ...
        return await deliveries_api.deliveries_deliveryId_get(
            request, deliveryId
        )

    async def patch(self, request, deliveryId):

        return await deliveries_api.deliveries_deliveryId_patch(
            request, deliveryId
        )

    async def delete(self, request, deliveryId):

        return await deliveries_api.deliveries_deliveryId_delete(
            request, deliveryId
        )


deliveries_if.add_route(
    deliveries_bydeliveryIdView.as_view(), "/deliveries/<deliveryId>"
)
